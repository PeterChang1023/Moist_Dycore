...
def ComputeVertEddy(v,t,p,p0=1e3,wave=0):
	...
	dp  = np.gradient(p, edge_order=2)[np.newaxis,:,np.newaxis]
	...
	if wave == 0:
		vp = GetAnomaly(v) # v = v'
		tp = GetAnomaly(t) # t = t'
		t = np.nanmean(vp*tp,axis=-1) # t = bar(v'Th')
	else:
		t = GetWaves(v,t,wave=wave,do_anomaly=True) # t = bar(v'Th'_{k=wave})
		if wave < 0:
			dthdp = np.expand_dims(dthdp,-1)
	t_bar = t/dthdp # np.nanmean(t, axis=0)/np.nanmean(dthdp, axis=0) # t_bar = bar(v'Th')/(dTh_bar/dp)
	#
	return v_bar,t_bar, dthdp, t
# def ComputeVertEddy(v, t, p, p0=1000, wave=0):
#     """
#     Computes residual circulation vertical eddy flux: bar(v'Theta') / d(bar Theta)/dp
...
#     # 1. Compute potential temperature
#     theta = t * (p0[np.newaxis, :, np.newaxis, np.newaxis] / p[np.newaxis, :, np.newaxis, np.newaxis])**kappa

#     # 2. Compute zonal means
#     v_bar = np.nanmean(v, axis=-1)
#     theta_bar = np.nanmean(theta, axis=-1)  # shape: [time, lev, lat]

#     # 3. Compute d(theta_bar)/dp averaged in time and zonal mean
#     theta_bar_mean = np.nanmean(theta_bar, axis=0)  # shape: [lev, lat]
#     dthdp = np.gradient(theta_bar_mean, p, axis=0, edge_order=2)  # shape: [lev, lat]

#     # 4. Compute anomalies (v', θ') along lon
#     vp = GetAnomaly(v) # v - np.nanmean(v, axis=3, keepdims=True)
#     thetap = GetAnomaly(theta) # theta - np.nanmean(theta, axis=3, keepdims=True)

#     # 5. Eddy flux <v'θ'>, zonal mean
#     vtheta_prime = np.nanmean(vp * thetap, axis=-1)  # shape: [time, lev, lat]

#     # 6. Expand dthdp for broadcasting
#     dthdp = np.where(np.abs(dthdp) < 1e-6, np.sign(dthdp) * 1e-6, dthdp)  # clip too-small slopes
#     dthdp_expand = dthdp[np.newaxis, :, :]  # shape: [1, lev, lat]

#     # 7. Final: bar(v'theta') / d(bar theta)/dp
#     vtheta_over_dthdp = vtheta_prime / dthdp_expand

#     return v_bar, vtheta_over_dthdp, dthdp, vtheta_prime
...
def ComputeEPfluxDiv(lat,pres,u,v,t,w=None,do_ubar=False,wave=0):
	...
	vbar,vertEddy, dthdp, vptp = ComputeVertEddy(v,t,pres,p0,wave) # vertEddy = bar(v'Th'/(dTh_bar/dp))
	#
	## get zonal anomalies
	u = GetAnomaly(u)
	v = GetAnomaly(v)
	...
	if wave < 0:
		div1 = 0 #coslat*np.gradient(ep1_cart,edge_order=2)[-2]/dphi - 2*sinlat*ep1_cart
	else:
		div1 = 0 #coslat*np.gradient(ep1_cart,edge_order=2)[-1]/dphi - 2*sinlat*ep1_cart
	...
	div2 = 0 # np.gradient(ep2_cart,edge_order=2)[1]/dp # [m/s2]
	#
	# convert to m/s/day
	div1 = div1*86400
	div2 = div2*86400
	#
	return ep1_cart,ep2_cart,div1,dthdp, vptp
...
def ComputeEPfluxDivXr(u,v,t,lon='infer',lat='infer',pres='infer',time='time',ref='mean',w=None,do_ubar=False,wave=0):
	...
	#
	## compute thickness weighted heat flux [m.hPa/s]
	vbar,vertEddy, t_bar = ComputeVertEddyXr(v,t,pres,p0,lon,time,ref,wave) # vertEddy = bar(v'Th'/(dTh_bar/dp))
	#
	## get zonal anomalies
	if isinstance(wave,list):
...
def PlotEPfluxArrows(x,y,ep1,ep2,fig,ax,xlim=None,ylim=None,xscale='linear',yscale='linear',invert_y=True, newax=False, pivot='tail',scale=None,quiv_args=None):
	...
	quivArgs = {'angles':'uv','scale_units':'inches','pivot':pivot, 'width':4e-3}
	...
	try:
		Q = ax.quiver(x,y,Fphi*dx,Fp*dy[:,np.newaxis],**quivArgs)
	except:
		print(dx.shape, Fphi.shape, dy.shape, Fp.shape);Q = ax.quiver(x,y,dx*Fphi,dy*Fp,**quivArgs)
	if scale is None:
